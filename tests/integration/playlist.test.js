...
describe('## Playlists API', () => {
    ...
    describe('# GET /api/playlists/156', () => {
        ...
        describe('# GET /api/playlists/156?fields[playlists]=title', () => {
            it('should return title in attributes', async () => {
                const res = await request.get('/api/playlists/156?fields[playlists]=title').set('api-key', process.env.API_KEY)
                expect(res.status).toBe(200)
                expect(Object.keys(res.body.data.attributes).join('')).toBe('title')
            })
        })

        describe('# GET /api/playlists/156/tracks?q=cinema', () => {
            it('should return data by query', async () => {
                const res = await request.get('/api/playlists/156/tracks?q=cinema').set('api-key', process.env.API_KEY)
                expect(res.status).toBe(200)
                expect(res.body.data).toBeInstanceOf(Array)
            })
        })
